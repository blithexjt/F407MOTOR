...
const AT_cmd_fun at_cmd_func[] = {
    {AT_CMD_SMPR,         "SMPR",          NULL},
...
u8 M8128_Config()
{
	...
	TIM_Delay*  delay;
	M8128_COMMAND  sgdm;
	M8128_COMMAND* sgdmptr = &sgdm;
	static command_sta com_sta = tx;
	if(tx == com_sta)
	{
		if(sem_valid != sem_wait(&gsem_event_flag))    return txwait;
		DeQueue(&list, &sgdm);
		SendCommand(sgdm);		
...
//  0xAA,0x55	   HB,LB          2B	(ChNum*N)B/6*4   1B/4B
	...
	if(0 == (pRxBuffer[i] == 0xAA && pRxBuffer[i+1] == 0x55))   return false;
